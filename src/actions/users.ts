...
export async function updateUserPassword(userId: string, newPassword: string) {
  try {
    ...
    // 3. Log Activity
    await logActivity('USER_UPDATE', `Changed password for user ${userId.slice(0, 8)}`, { target_user_id: userId })
...
export async function updateUserRole(userId: string, newRole: 'user' | 'owner') {
  try {
    ...
    // Log
    await logActivity('USER_UPDATE', `Changed role of user ${userId.slice(0,8)} to ${newRole}`, { target_user_id: userId, new_role: newRole })
...
export async function deleteUser(userId: string, email?: string) {
    try {
        ...
        // Log
        await logActivity('USER_UPDATE', `Deleted user ${targetEmail}`, { target_user_id: userId })
