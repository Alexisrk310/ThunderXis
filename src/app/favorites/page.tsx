    ...
    if (!user) {
        return (
            <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center">
                <h1 className="text-3xl font-bold mb-4">{t('nav.favorites')}</h1>
                <p className="text-muted-foreground mb-8 text-lg">{t('favorites.login_msg')}</p>
        ...
        <div className="min-h-screen bg-background pt-24 pb-12 px-6">
            <div className="max-w-7xl mx-auto">
                <h1 className="text-4xl font-bold mb-12 flex items-center gap-3">
                    <span className="text-red-500">❤️</span> {t('nav.favorites') || 'My Favorites'}
                </h1>

                {favorites.length === 0 ? (
                    <div className="text-center py-20 bg-card/30 rounded-3xl border border-white/5">
                        <p className="text-xl text-muted-foreground mb-6">{t('favorites.empty')}</p>
